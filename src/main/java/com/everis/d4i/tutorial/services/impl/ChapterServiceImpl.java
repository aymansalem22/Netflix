...
public class ChapterServiceImpl implements ChapterService {
	...
	@Override
	public ChapterRest updateChapterNameById(Long chapterId, String chapterName) throws NetflixException {
		Optional<Chapter> chapter = chapterRepository.findById(chapterId);
		if (chapter.isPresent()) {
			Chapter chapterExists = chapter.get();
			chapterExists.setName(chapterName);
			chapterExists = chapterRepository.save(chapterExists);
			return modelMapper.map(chapterExists, ChapterRest.class);
		}
		throw new NotFoundException("chapter Id not found " + chapterId);
	}

	@Override
	public Chapter findChapterById(Long idChapter) throws NetflixException {
		Optional<Chapter> chapter = chapterRepository.findById(idChapter);
		if (chapter.isPresent()) {
			return chapter.get();
		}
		throw new NotFoundException("chapter Id not found " + idChapter);
